...
RegisterNetEvent('humrp_robbery:onPedDeath')
AddEventHandler('humrp_robbery:onPedDeath', function(store)
    SetEntityHealth(peds[store], 0)
end)

RegisterNetEvent('humrp_robbery:removePickup')
AddEventHandler('humrp_robbery:removePickup', function(bank)
    for i = 1, #objects do 
...
RegisterNetEvent('humrp_robbery:robberyOver')
AddEventHandler('humrp_robbery:robberyOver', function()
    robbing = false
end)

RegisterNetEvent('humrp_robbery:talk')
AddEventHandler('humrp_robbery:talk', function(store, text, time)
    robbing = false
...
RegisterNetEvent('humrp_robbery:rob')
AddEventHandler('humrp_robbery:rob', function(i)
                    ...
                    while true do
                        ...
                        if DoesEntityExist(bag) then
                            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(bag), true) <= 1.5 then
                                ...
                                TriggerServerEvent('humrp_robbery:pickUp', i)
                                break
                            end
                        else
                            break
...
RegisterNetEvent('humrp_robbery:resetStore')
AddEventHandler('humrp_robbery:resetStore', function(i)
   ...
   while not ESXLoaded do Wait(0) end
    ...
    DeletePed(peds[i])
    Wait(550)
    peds[i] = _CreatePed(Config.ShopOwner, Config.Shops[i].coords, Config.Shops[i].heading)
...
Citizen.CreateThread(function()
    ...
    Citizen.CreateThread(function()
        while true do
            for i = 1, #peds do
                if IsPedDeadOrDying(peds[i]) then
                    TriggerServerEvent('humrp_robbery:pedDead', i)
                end
            end
            Wait(5000)
        end
                        ...
                        if not robbing then
                            local canRob = nil
                                ...
                                if GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 7.5 then
                                    if not IsPedDeadOrDying(peds[i]) then
                                        TriggerServerEvent('humrp_robbery:rob', i)
                                        while robbing do Wait(0) if IsPedDeadOrDying(peds[i]) then robbing = false end  end
                                    end
                                else
